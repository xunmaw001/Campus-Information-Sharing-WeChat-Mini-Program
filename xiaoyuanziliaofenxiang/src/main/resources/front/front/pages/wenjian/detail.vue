...
<script>
	export default {
		...
		methods: {
			...
			async download(url) {
				let _this = this;
				if(_this.user.yonghuJifenNumber <=0){
					_this.$utils.msg('积分不足不能下载');
					return false
				}
				_this.user.yonghuJifenNumber = Number(_this.user.yonghuJifenNumber)-Number(_this.detail.wenjianJifenNumber)
				await _this.$api.update(`yonghu`, _this.user);
				let yonghu = await this.$api.info('yonghu', _this.detail.yonghuId);
				yonghu.data.yonghuJifenNumber = Number(yonghu.data.yonghuJifenNumber)+Number(_this.detail.wenjianJifenNumber)
				await _this.$api.update(`yonghu`, yonghu.data);
				uni.downloadFile({
					url: url,
					success: (res) => {
