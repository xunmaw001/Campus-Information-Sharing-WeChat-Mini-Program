   	 ...
   	 <view style="margin-top: 10rpx;"></view>
					...
					<view class="detail-list-item"
						...
						<view class="text" :style='{"color":"#f06f4f","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.wenjianCollectionValue}}</view>
					</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('wenjianCollection', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.wenjianCollectionPhoto ? this.detail.wenjianCollectionPhoto.split(",") : [];
			},
